...
class SpawnSpec {
  ...
  /// Model passed as `--model` at spawn. Only read for
  /// [SessionProfile.companion].
  ///
  /// An agent session sets its model with a `set_model` control request after
  /// spawn ([applySessionDefaults]), which is right for it: the user can change
  /// it mid-session from the picker. The companion cannot — and doing it that
  /// way put **three backstage messages at the head of his context**, found by
  /// reading his transcript on the first live run:
  ///
  /// ```
  ...
  /// <local-command-stdout>Set model to haiku…
  /// ```
  ///
  /// The CLI echoes a control request into the conversation as a local command,
  /// so the first thing Clide ever read was a slash command and its output —
  /// directly above the rule forbidding him to mention that such things exist.
  /// A flag is silent.
  final String? model;
