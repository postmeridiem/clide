...
def set_event_callback(callback: Optional[Callable[[str], None]]) -> None:
    ...
    Useful for Claude Code event detection.

    Args:
        callback: A callable that accepts raw terminal data, or None to disable.
    """
...
class Stream:
    ...
    def feed(self, data: str) -> None:
        ...
        """
        # Clide: Invoke event callback for raw data
        if _event_callback is not None:
            try:
                _event_callback(data)
            except Exception:
                pass  # Don't let callback errors affect parsing
    ...
    def _parser_fsm(self) -> ParserGenerator:
        ...
        while True:
            ...
            if char in basic:
                ...
                while True:
                    ...
                    if char == "?":
                        ...
                        if char == ";":
                            ...
                            # Clide: Log CSI sequence
                            if _debug_logger is not None:
                                _log_debug(f"[CSI] char={char!r} params={params} private={private}")
                ...
                # Clide: Log OSC sequence
                if _debug_logger is not None:
                    _log_debug(f"[OSC] code={code!r} param={param!r}")
