...
- **Flutter desktop is the host. No Electron, ever.** The web UI is a real target: the full IDE in a browser, served from a containerised host ([`D-116`](governance/decisions/architecture.md#d-116-web-ui-mode--full-clide-in-the-browser-served-from-a-containerised-host), front door [`D-117`](governance/decisions/architecture.md#d-117-web-front-door--caddy-at-the-edge-internal-dart-broker-and-hosts)). It is the same Flutter UI, and desktop fidelity is never traded for it. The web build is Flutter's **WebAssembly (CanvasKit/Skwasm) compile**, not the JS/HTML renderer. `xterm.dart` is the terminal renderer; markdown, canvas, graph are custom `CustomPaint`/widget components.
- **Single process — per host, per workspace.** On desktop the Flutter app hosts everything in-process: IPC server, subsystem handlers (pane, files, editor, git, pql), extensions. No separate daemon binary (D-56 dissolved it). In web UI mode the UI runs in the browser, and each workspace's subsystems run in one host process behind the front door (D-116).
